<template>
    <div id = "root">
        <div class="todo-container">
            <div class="todo-wrap">
                <MyHeader/>
                <Todos :todos="todos"/>
                <MyFooter :todos="todos"/>
            </div>
        </div>
    </div>
</template>

<script>
    import pubsub from 'pubsub-js'
    import MyHeader from './components/MyHeader.vue'
    import MyFooter from './components/MyFooter.vue'
    import Todos from './components/Todos.vue'
    export default {
        name:'App',
        components:{MyHeader, MyFooter, Todos},
        data(){
            return{
                //读取本地浏览器中保存的数据
                todos:JSON.parse(localStorage.getItem('todos'))||[]
            }
        },
        watch:{
            todos:{
                deep:true,
                handler(value){
                     localStorage.setItem('todos',JSON.stringify(value))
                }
            }
        },
        methods:{
            //添加一个todo
            addTodo(_,todo){
                console.log('App收到了消息订阅')
                this.todos.unshift(todo)
            },

            //勾选or取消勾选一个todo
            checkTodo(id){
                this.todos.forEach((todo)=>{
                    if(todo.id=== id){
                        todo.done = !todo.done
                    }
                })
            },

            //编辑一个Todo
            editedTode(tId,tTitle){
                this.todos.forEach((todo)=>{
                    if(todo.id === tId)
                    todo.title = tTitle
                })
            },

            //删除所选的Todo
            deleteTode(id){
                this.todos = this.todos.filter(todo => todo.id !== id)
            },

            //全选or取消全选
            checkedAllTodo(done){
                this.todos.forEach((todo)=>{
                    todo.done = done
                })
            },

            //清除所有已经完成了的Todo
            clearDoneTodo(){
                this.todos = this.todos.filter((todo)=>!todo.done)
            }
        },
        mounted(){
            //消息订阅写法
            const pubId = pubsub.subscribe('addTodo',this.addTodo)

            //总线写法
            // this.$bus.$on('addTodo',this.addTodo)

            this.$bus.$on('checkTodo',this.checkTodo)
            this.$bus.$on('deleteTode',this.deleteTode)
            this.$bus.$on('editedTode',this.editedTode)
            this.$bus.$on('checkedAllTodo',this.checkedAllTodo)
            this.$bus.$on('clearDoneTodo',this.clearDoneTodo)
        },
        beforeDestroy(){
            pubsub.unsubscribe(pubId)
            // this.$bus.$off('addTodo')
            this.$bus.$off('checkTodo')
            this.$bus.$off('deleteTode')
            this.$bus.$off('editedTod')
            this.$bus.$off('checkedAllTodo')
            this.$bus.$off('clearDoneTodo')
        }
    }
</script>

<style>
    body{
        background-color: #fff;
    }

    .btn{
        display: inline-block;
        padding:4px 12px;
        margin:0 2px;
        font-size:14px;
        line-height:20px;
        text-align: center;
        vertical-align: middle;
        cursor: pointer;
        box-shadow: inset 0 1px 0 rgba(255, 255, 255, 0.2),0 1px 2px rgba(0, 0, 0, 0.05);
        border-radius: 4px;
    }

    .btn-danger{
        color:#fff;
        background-color: #da4f49;
        border: 1px solid #bd362f
    }

    .btn-edit{
        color:#fff;
        background-color: rgb(44, 228, 44);
        border: 1px solid rgb(31, 177, 31);
    }

    .btn-danger:hover{
        color:#fff;
        background:#bd362f
    }

    .btn:focus{
        outline:none;
    }

    .todo-container{
        width:600px;
        margin:0 auto;
    }

    .todo-container .todo-wrap {
        border: 1px solid #ddd;
        padding:10px;
        border-radius:5px;
    }
</style>